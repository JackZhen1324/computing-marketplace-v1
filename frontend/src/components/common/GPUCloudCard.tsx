...
interface GPUCloudCardProps {
  product: Product;
}

const GPUCloudCard = ({ product }: GPUCloudCardProps) => {
  ...
  const vcpu = product.specifications.find(spec => spec.label === 'vCPU')?.value || 'N/A';
  ...
  const formatPrice = (price: string) => {
    if (price.includes('小时')) {
      const match = price.match(/¥(\d+)/);
      if (match) {
        return `¥${match[1]}.00/小时`;
      }
    }
    return price;
  };
  ...
  return (
    <Card
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = '0 12px 32px rgba(245, 87, 108, 0.2)';
    ...
    >
      ...
      <div style={{ marginBottom: '24px' }}>
        {[
          ...
          { label: 'vCPU', value: vcpu, icon: <CheckOutlined /> },
          ...
          >
            <span style={{ color: '#f5576c', fontSize: '14px' }}>
              {spec.icon}
            </span>
            <span style={{ color: '#999', minWidth: '60px' }}>{spec.label}：</span>
